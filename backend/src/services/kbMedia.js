...
async function ensureKbMediaSchema() {
  if (ready) return;
  await pool.query(`
  ...
  `);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_kb_connectors_created ON kb_connectors(created_at DESC)`);
  ready = true;
}

module.exports = { ensureKbMediaSchema };
